...
def consumer_sum():
    ...
    plt.plot(
        ...
        list(map(
            ...
            hospital, factory,
            *(map(lambda forecast: forecast + 0.82 * (5 - price) ** 2.6, houseA) for price in houseA_prices),
            *(map(lambda forecast: forecast + 0.24 * (9 - price) ** 2.2, houseB) for price in houseB_prices)
        ))
    )
